<?php
    ...
    //Bloqueando página para usuarios não logados
        //1 - obtendo sessao iniciada
    session_start();
        //2 - aplicando validação para redirecionar usuario caso nao esteja logado, bloqueando o acesso a essa pagina
    ...
    //excluindo usuários
     ...
     if(isset($_GET) && $_GET){
        ...
        //4 - percorrendo array que contem a lista de usu[arios
        foreach ($arrayUsuarios['usuarios'] as $chave => $usuario) {
            // verificando se encontramos o usuario para fazer a exlcusao
            if($usuario["id"] == $id){
        ...
        // 5 - transformar o conteudo do arquivo usuarios.json que foi alterado em uma string json
        $jsonUsuarios = json_encode($arrayUsuarios);

        // 6 - sobrescrevendo o conteudo do arquivo usuarios.json
        ...
        <article class="row">
            ...
            <div class = 'col-md-12'>
                <?php 
                    if(isset($_GET) && $_GET){
                        if ($excluiu){
                            echo "<div class='col-md-12 alert-success'> Usuário excluido com sucesso</div>";
                        } else {
                            echo "<div class='col-md-12 alert-danger'> Falha ao processar requsição</div>";
                        }
                    }
                ?>
            </div>
